...
( supervisor_loop "httpd" "$MODDIR/www/start-httpd.sh" 15 ) &

# Telegram bot — token + chat_id bekler (bot.sh kendi bekler)
( supervisor_loop "bot" "$MODDIR/bot/bot.sh" 15 ) &

# xray yalnızca kullanıcı etkinleştirmişse (route mode'a göre tun0/tproxy)
if [ "$(cat "$DATA/conf/xray_enabled" 2>/dev/null)" = "1" ]; then
    "$MODDIR/lib/action.sh" xray_start >> "$LOG" 2>&1
fi
```
> Not: `supervisor_loop` imzası bin-utils'ta `supervisor_loop <ad> <komut> <gecikme>`; gerçek imzayı `grep -n supervisor_loop /data/adb/modules/bin-utils/lib/common.sh` ile doğrula ve eşle.
